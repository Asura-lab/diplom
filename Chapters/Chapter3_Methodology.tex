...
\begin{tikzpicture}[scale=0.9, transform shape,
    ...
    % === LAYER LABELS ===
    \node[font=\sffamily\small\bfseries, violet!60!black] at (0,2) {Application};
    \node[font=\sffamily\small\bfseries, blue!60!black] at (4,2) {Backend};
    ...
    % ML Models - V10: 7 models (XGB×3, LGB×2, CAT×2)
    \node[box=green!70!black, minimum width=1.5cm] (xgb) at (7.2,1.5) {\footnotesize XGB×3};
    \node[box=yellow!70!black, minimum width=1.5cm] (lgb) at (9.2,1.5) {\footnotesize LGB×2};
    \node[box=orange!70!black, minimum width=1.5cm] (cat) at (11.2,1.5) {\footnotesize CAT×2};
    \node[box=red!50!black, minimum width=1.4cm] (bonus) at (13.2,1.5) {\footnotesize +Bonus};
    ...
    % === ENSEMBLE LABEL ===
    \node[font=\sffamily\scriptsize\itshape, green!40!black] at (10,0.7) {V10 Ensemble (7 Models + Agreement Bonus)};
...
Энэхүү судалгаанд долоон машин сургалтын моделийн ensemble ашигласан бөгөөд энэ нь V10 (Version 10) гэж нэрлэгдсэн. Гурван ялгаатай алгоритмын олон хувилбарыг нэгтгэснээр илүү найдвартай таамаглал гаргана:

\begin{enumerate}
    \item \textbf{XGBoost × 3} - Gradient Boosting Decision Trees (гурван өөр hyperparameter)
    \item \textbf{LightGBM × 2} - Light Gradient Boosting Machine (хоёр өөр тохиргоо)
    \item \textbf{CatBoost × 2} - Categorical Boosting (хоёр өөр тохиргоо)
\end{enumerate}

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    block/.style={rectangle, draw, rounded corners, minimum width=1.8cm, minimum height=0.7cm, fill=blue!20, font=\small},
    arrow/.style={-Stealth, thick},
    node distance=0.8cm
]
    % Input
    \node[block, fill=gray!20, minimum width=4cm] (input) {32 Technical Features};
    
    % Models - Row 1 (XGBoost variants)
    \node[block, below left=1.5cm and 2.5cm of input, fill=green!25] (xgb1) {XGBoost-1};
    \node[block, below left=1.5cm and 0.5cm of input, fill=green!20] (xgb2) {XGBoost-2};
    \node[block, below=1.5cm of input, fill=green!15] (xgb3) {XGBoost-3};
    
    % Models - Row 1 (LightGBM and CatBoost)
    \node[block, below right=1.5cm and 0.5cm of input, fill=yellow!25] (lgb1) {LightGBM-1};
    \node[block, below right=1.5cm and 2.5cm of input, fill=yellow!15] (lgb2) {LightGBM-2};
    
    % CatBoost models
    \node[block, below=3.2cm of xgb2, fill=orange!25] (cat1) {CatBoost-1};
    \node[block, below=3.2cm of lgb1, fill=orange!15] (cat2) {CatBoost-2};
    
    % Agreement Bonus
    \node[block, below=4.5cm of input, fill=cyan!20, minimum width=5cm] (agreement) {Agreement Bonus System};
    
    % Ensemble
    \node[block, below=0.8cm of agreement, fill=red!20, minimum width=5cm] (ensemble) {Weighted Ensemble + Bonus};
    
    % Output
    \node[block, below=0.8cm of ensemble, fill=purple!20, minimum width=3cm] (output) {BUY Confidence};
    
    % Arrows - Input to models
    \draw[arrow] (input) -- (xgb1);
    \draw[arrow] (input) -- (xgb2);
    \draw[arrow] (input) -- (xgb3);
    \draw[arrow] (input) -- (lgb1);
    \draw[arrow] (input) -- (lgb2);
    
    % XGB to CatBoost area
    \draw[arrow] (xgb1) |- (cat1);
    \draw[arrow] (xgb3) |- (cat1);
    \draw[arrow] (lgb1) |- (cat2);
    \draw[arrow] (lgb2) |- (cat2);
    
    % To agreement
    \draw[arrow] (cat1) -- (agreement);
    \draw[arrow] (cat2) -- (agreement);
    \draw[arrow] (xgb2) -- (agreement);
    
    % Final flow
    \draw[arrow] (agreement) -- (ensemble);
    \draw[arrow] (ensemble) -- (output);
\end{tikzpicture}
\caption{V10 Ensemble моделийн архитектур (7 загвар)}
\label{fig:ensemble_arch}
\end{figure}

\subsection{XGBoost загварууд}

XGBoost (eXtreme Gradient Boosting) нь gradient boosting framework дээр суурилсан алгоритм юм. V10-д гурван өөр hyperparameter-тэй XGBoost загварыг ашигласан:
...
V10-ийн гол шинэлэг тал бол загваруудын ``зөвшилцөл''-д суурилсан нэмэлт оноо юм. Бүх загварууд ижил таамаглал гаргах тусам итгэлцүүр нэмэгддэг:
...
\begin{tabular}{|c|c|l|}
\hline
\textbf{Зөвшилцөл} & \textbf{Нэмэлт оноо} & \textbf{Тайлбар} \\
\hline
7/7 загвар & +7\% & Бүгд ижил таамаглал \\
\hline
6/7 загвар & +4\% & Нэгээс бусад нь ижил \\
\hline
5/7 загвар & +2\% & Олонхи нь ижил \\
\hline
\end{tabular}
...
\begin{table}[H]
\centering
\caption{V10 Моделийн Hyperparameter тохиргоо}
\begin{tabular}{|l|l|l|l|l|}
\hline
\textbf{Параметр} & \textbf{XGB-1} & \textbf{XGB-2/LGB-1} & \textbf{LGB-2} & \textbf{CatBoost} \\
\hline
...
V10 моделийн итгэлцүүрийн босго нь дохиог үүсгэх эсэхийг тодорхойлно:

\begin{itemize}
    \item $P_{BUY} \geq 85\%$: BUY дохио үүсгэнэ (96.9\% нарийвчлал)
    \item $P_{BUY} \geq 80\%$: BUY дохио (94.7\% нарийвчлал)
    \item $P_{BUY} < 80\%$: HOLD (хүлээх)
\end{itemize}

85\% босго нь хамгийн оновчтой бөгөөд 96.9\% accuracy, 52.2 profit factor-тай байна.
